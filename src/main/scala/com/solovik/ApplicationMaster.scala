...
class ApplicationMaster extends AMRMClientAsync.CallbackHandler {
  ...
  override def onContainersAllocated(containers: util.List[Container]): Unit = {
    ...
    val commands = List(
      "$JAVA_HOME/bin/java " +
        " -Xmx256m " +
        s" com.solovik.DummyApplication " +
    ...
    val localResources = Map(
      "SolovikYarnApp-assembly-0.1.jar" -> setUpLocalResourceFromPath(FileSystem.get(conf).makeQualified(new Path(sys.env("SOLOVIK_JAR_PATH"))))
    )
