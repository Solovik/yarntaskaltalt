...
import com.solovik.YarnAppUtils._
...
object SampleYarnClient {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val commands = List(
      "$JAVA_HOME/bin/java " +
        " -Xmx256m " +
        s" com.solovik.ApplicationMaster " +
    ...
    val localResources = Map(
      "SolovikYarnApp-assembly-0.1.jar" -> setUpLocalResourceFromPath(yarnPath) //Give the yarn path as local resource so that the container can download this and use it locally
    )

    val addlEnvVars = Map (
      "SOLOVIK_JAR_PATH" -> yarnPath.toUri.getRawPath
    )
